...
SELECT 
    COUNT(*) AS Total_director_mapping
FROM
    director_mapping;-- Total count 3867
SELECT 
    COUNT(*) AS Total_genre
FROM
    genre;-- Total count 14662
SELECT 
    COUNT(*) AS Total_movie
FROM
    movie;-- Total count 7997
SELECT 
    COUNT(*) AS Total_names
FROM
    names;-- Total count 25735
SELECT 
    COUNT(*) AS Total_ratings
FROM
    ratings;-- Total count 7997
SELECT 
    COUNT(*) AS Total_role_mapping
FROM
    role_mapping;  -- Total count 15615
...
/* We  have 4 columns with null values and they are 
- country with 20 null values
- worlwide_gross_income with 3724 values
- languaages with 194 null vaalues
- production_compaany with 528 null values
*/
...
/* We have use like % as we have seen in data where some of the movies are produce in other countries along with iNDIA & USA and 
some movies only produced in INDIA and USA 
*/
...
SELECT DISTINCT
    genre AS unique_list
FROM
    genre;

/* The unique_list are as follows
   Thriller
   Sci-Fi
   Romance
   Others
...
/* Out of all genre Action has the highest avg_duration with 112.88 mins
*/
...
WITH production_hit_movie_summary AS
	(
		SELECT 
...
SELECT 
    country, SUM(total_votes) AS count_votes
FROM
    movie AS m
        INNER JOIN
...
SELECT 
	name AS actor_name,SUM(total_votes)AS total_votes,
...
AS
	...
	SELECT
		...
		DENSE_RANK() OVER(PARTITION BY year ORDER BY convert(REPLACE(TRIM(worlwide_gross_income), "$ ",""),UNSIGNED int) DESC) AS movie_rank
	FROM 
		movie m
	INNER JOIN 
		genre g
...
SELECT 
	...
	SUM(total_votes) AS total_votes, COUNT(rm.movie_id) AS movie_count, 
